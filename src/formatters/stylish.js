...
export default (diff, indentChar = ' ', indentCharsCount = 4) => {
  ...
  const iter = (data, depth) => {
    ...
    const formattedLines = data.reduce((acc, { value1, value2, key, type, children }) => {
      switch (type) {
        case 'nested':
          return [...acc, `${indent}${key}: ${iter(children, depth + 1)}`];
        case 'unchanged':
          return [...acc, `${indent}${key}: ${formatValue(value1, depth + 1)}`];
        case 'updated':
          return [
            ...acc,
        ...
        case 'removed':
          return [...acc, `${formatIndent(indent, '-')}${key}: ${formatValue(value1, depth + 1)}`];
        case 'added':
          return [...acc, `${formatIndent(indent, '+')}${key}: ${formatValue(value2, depth + 1)}`];
        default:
